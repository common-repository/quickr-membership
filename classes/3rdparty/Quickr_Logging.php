...
class Quickr_Logging {
    ...
    function __construct() {
        ...
        add_action('quickr_logging_prune_routine', array($this, 'prune_logs'));
        add_filter('the_content', array($this, 'the_content'));
        add_filter('bulk_actions-edit-wp_log', array($this, 'bulk_actions'));
    }
    ...
    public function the_content($content) {
        ...
        if (isset($post->post_type) && $post->post_type == 'wp_log') {
            ...
            foreach ($metas as $meta_key => $meta_value) {
                $output .= sprintf("%50s : %s", substr($meta_key, 8), $meta_value[0]) . PHP_EOL;
            }
            return $content . $output . '</pre>';
        }
    ...
    public function prune_logs() {
        ...
        $should_we_prune = apply_filters('quickr_logging_should_we_prune', false);
...
// prune_logs
     ...
     *
     * @filter quickr_logging_force_delete_log         Allows user to override the force delete setting which bypasses the trash
     */
    private function prune_old_logs($logs) {

        $force = apply_filters('quickr_logging_force_delete_log', true);
...
// prune_old_logs
     ...
     *
     * @filter quickr_logging_prune_when           Users can change how long ago we are looking for logs to prune
     * @filter quickr_logging_prune_query_args     Gives users access to change any query args for pruning
     */
    private function get_logs_to_prune() {

        $how_old = apply_filters('quickr_logging_prune_when', '2 weeks ago');
        ...
        $old_logs = get_posts(apply_filters('quickr_logging_prune_query_args', $args));
...
// get_logs_to_prune
    ...
    private static function log_types() {
        ...
        return apply_filters('quickr_log_types', $terms);
    }
    ...
    public function register_post_type() {
        ...
        $log_args = array(
            'labels' => array('name' => __('Logs', 'quickr-logging')),
        ...
        register_post_type('quickr_log', apply_filters('quickr_logging_post_type_args', $log_args));
    }
    ...
    public function register_taxonomy() {
        ...
        register_taxonomy('quickr_log_type', 'quickr_log', array('public' => defined('WP_DEBUG') && WP_DEBUG));

        $types = self::log_types();

        foreach ($types as $type) {
            if (!term_exists($type, 'quickr_log_type')) {
                wp_insert_term($type, 'quickr_log_type');
            }
        }
    }
     ...
     */
    public static function add($title = '', $message = '', $parent = 0, $type = null, $log_meta = array()) {
        ...
        return self::insert_log($log_data, $log_meta);
    }
    ...
    public static function insert_log($log_data = array(), $log_meta = array()) {
        ...
        do_action('quicrk_pre_insert_log');
        ...
        if ($log_data['log_type'] && self::valid_type($log_data['log_type'])) {
            wp_set_object_terms($log_id, $log_data['log_type'], 'quickr_log_type', false);
        }
        ...
        if ($log_id && !empty($log_meta)) {
            foreach ((array) $log_meta as $key => $meta) {
                update_post_meta($log_id, '_quickr_log_' . sanitize_key($key), $meta);
            }
        }

        do_action('quickr_post_insert_log', $log_id);
    ...
    public static function update_log($log_data = array(), $log_meta = array()) {
        ...
        do_action('quickr_pre_update_log', $log_id);
        ...
        do_action('quickr_post_update_log', $log_id);
    }
...
$GLOBALS['quickr_logs'] = new Quickr_Logging();
